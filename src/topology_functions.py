...
def get_adjacency_matrix_from_graph(
        G):  # if two qubits need to be connected, there is a 1, the diagonal is 0. Since we are dealing with undirected graphs, the matrix is symmetric.
...
def create_heavy_hex_IBMQ(n_unit_rows, n_columns):
    ...
    for row in range(n_rows):
        ...
        if 0 < row:
            leftmost_qubit_index = (n_qubits_row - 1 + n_qubits_intermediate) + (row - 1) * (
                        n_qubits_intermediate + n_qubits_row)
...
def get_extended_heavy_hex_IBMQ(coupling_map_initial, extra_rows, extra_columns):
    ...
    if len(coupling_map_initial) > 0:
        ...
        n_unit_cells_vert = np.round(
            (n_qubits_initial - (4 * n_unit_cells_hor + 1)) / (5 * n_unit_cells_hor + 4)).astype(int)
...
def get_qubit_coordinates_heavy_hex(n_unit_rows, n_columns, zoom):
    ...
    for row in range(n_rows):
        ...
        if row < n_rows - 1 and n_unit_rows == 1:
            for i in range(qubit_line_length):
                qubit_coordinates.append([y0 * zoom, (
                            i + 1) * zoom])  # if there is only one unit cell, the positions are shifted by 1 in the first row to avoid an x-position of -1 in the last row
...
def increase_coupling_density(base_map, density):
    ...
    eligible_edges = get_eligible_edges(base_map, coupling_map_distances,
                                        distance)  # get all qubits which have the specified distance of 2
...
def create_sycamore_topology(rows, columns):
    connectivity_map = []
    for i in range(rows):
        for j in range(columns):
            qubit = i * columns + j
            if j < columns - 1:
                connectivity_map.append([qubit, qubit + 1])
                connectivity_map.append([qubit + 1, qubit])
            if i < rows - 1:
                connectivity_map.append([qubit, qubit + columns])
                connectivity_map.append([qubit + columns, qubit])
